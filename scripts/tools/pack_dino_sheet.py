...
res_path = "res://" + OUT if not OUT.startswith("res://") else OUT
print("\n--- paste into dino.gd (replaces the %s entry) ---" % DINO)
print('const %s := "%s"' % (const_name, res_path))
print('\t"%s": {' % DINO)
print('\t\t"sheet": %s,' % const_name)
print('\t\t"motion": true,   # 3D-baked frames -> wants_rig=false, AnimatedSprite2D plays these')
